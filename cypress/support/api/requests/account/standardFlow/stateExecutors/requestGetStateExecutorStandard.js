...
class RequestGetStateExecutorStandard {

    getAllStateExecutors(tokenAuth) {
        return cy.request({
            methods: 'GET',
    ...
    getNStateExecutors(tokenAuth) {
        return cy.request({
            methods: 'GET',
            url: `${Config.DEV_API_GATEWAY}/v1/stateexecutors/?page=1&limit=20`,
            headers: {
                'content-type': 'application/json',
                'Authorization': `Bearer ${tokenAuth.body.access_token}`
            }
    ...
    getOrderedStateExecutors(tokenAuth) {
        //Retorna todos os protetores ordenados por ordem crescente
        return cy.request({
            methods: 'GET',
            url: `${Config.DEV_API_GATEWAY}/v1/stateexecutors/?sort=+created_at`, 
            headers: {
                'content-type': 'application/json',
                'Authorization': `Bearer ${tokenAuth.body.access_token}`
            }
    ...
    getFilteredStateExecutors(tokenAuth) {
        ...
        return cy.request({
            methods: 'GET',
            url: `${Config.DEV_API_GATEWAY}/v1/stateexecutors?start_at=${anoAtual}-${mesAtual - 1}-01&end_at=${anoAtual}-${mesAtual}-31`,
            headers: {
                'content-type': 'application/json',
                'Authorization': `Bearer ${tokenAuth.body.access_token}`
            }
...
export const requestGetStateExecutorStandard = new RequestGetStateExecutorStandard();